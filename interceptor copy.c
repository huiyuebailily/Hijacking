...
asmlinkage long interceptor(struct pt_regs reg) {
	...
	if (table[reg.ax].monitored == 2 || check_pid_monitored(reg.ax,current->pid) == 1){
		//printk("syscall is monitored ,no=%p,%p,%p,%p,%p\n",reg.ax,reg.bx,reg.cx,reg.si,reg.di);
		//log_message();
		//log_message(current->pid, MY_CUSTOM_SYSCALL, reg.ax, reg.bx, reg.cx, reg.dx, reg.si, reg.di);
		log_message(current->pid, reg.ax, reg.bx, reg.cx, reg.dx, reg.si, reg.di,reg.bp);
	}
	else {
		//printk("syscall is not monitored ,no=%d,%d,%d,%d,%d\n",reg.ax,reg.bx,reg.cx,reg.si,reg.di);
	}
	//
	
	return  table[reg.ax].f(reg);

	//sys_call = table[reg.ax].f(reg);
	//sys_call(reg);
...
asmlinkage long my_syscall(int cmd, int syscall, int pid) {
	...
	{
		printk("Bad cmd =%d,. It must be 1,2,3,4\n",cmd);
	...
	{
		printk("Invalid syscall=%d,. It must be >0 and < %d\n",syscall,NR_syscalls);
		spin_unlock(&sys_call_table_lock  );
		spin_unlock(&my_table_lock );
		return  -EINVAL;
	}
	...
	if (cmd == REQUEST_SYSCALL_INTERCEPT || cmd == REQUEST_SYSCALL_RELEASE ){
		if (current_uid()  != 0){
			printk("IREQUEST_SYSCALL_INTERCEPT and  REQUEST_SYSCALL_RELEASE  need root privilege\n");
			spin_unlock(&sys_call_table_lock  );
			spin_unlock(&my_table_lock );
			return  -EPERM;
		}
	...
	if (cmd == REQUEST_START_MONITORING || cmd == REQUEST_STOP_MONITORING ){
		if (table[syscall].intercepted != 1){
			printk("The system cal =%d is not been montored \n",cmd);
			spin_unlock(&sys_call_table_lock  );
			spin_unlock(&my_table_lock );
			return  -EINVAL;
		}
	...
	if (cmd == REQUEST_START_MONITORING || cmd == REQUEST_STOP_MONITORING ) {
	    if (current_uid()  != 0 ){
		if (pid == 0){
			printk("REQUEST_START_MONITORING and  REQUEST_STOP_MONITORING for pid=0 need root privilege\n");
			spin_unlock(&sys_call_table_lock  );
			spin_unlock(&my_table_lock );
			return  -EPERM;
		}
		if (pid < 0){
			printk("REQUEST_START_MONITORING and  REQUEST_STOP_MONITORING for pid=%d is invalid\n",pid);
			spin_unlock(&sys_call_table_lock  );
			spin_unlock(&my_table_lock );
			return  -EINVAL;
		}
		if (pid != current->pid){
			printk("REQUEST_START_MONITORING and  REQUEST_STOP_MONITORING for pid=%d need root or itself\n",
				pid);
			spin_unlock(&sys_call_table_lock  );
			spin_unlock(&my_table_lock );
			return  -EPERM;
		...
		if (cmd == REQUEST_START_MONITORING){
			if (check_pid_monitored(syscall,pid)  == 1){
				printk("The process pid=%d has been monitored\n",pid);
				spin_unlock(&sys_call_table_lock  );
				spin_unlock(&my_table_lock );
				return  -EBUSY;
			}
		...
		if (cmd == REQUEST_STOP_MONITORING ){
			if (check_pid_monitored(syscall,pid)  != 1){
				printk("The process pid=%d has not been monitored, can not stop monitor\n",pid);
				spin_unlock(&sys_call_table_lock  );
				spin_unlock(&my_table_lock );
				return  -EINVAL;
			}
		...
		if (cmd == REQUEST_START_MONITORING){
			if (check_pid_monitored(syscall,pid)  == 1){
				...
				if (pid == 0) table[syscall].monitored = 2; //moniter all pid
				else if (pid < 0){
					printk("The process pid=%d is invalid\n",pid);
					spin_unlock(&sys_call_table_lock  );
					spin_unlock(&my_table_lock );
					return  -EINVAL;
				}
		...
		if (cmd == REQUEST_STOP_MONITORING ){
			if (pid != 0 && check_pid_monitored(syscall,pid)  != 1 ){
				...
				if (pid == 0) {
					table[syscall].monitored = 0; //demoniter all pid
					destroy_list(syscall);
				}
				else {
					del_pid_sysc(pid, syscall);
					//able[syscall].monitored = 1;
				}
	...
	if (cmd == REQUEST_SYSCALL_INTERCEPT){
		if (table[syscall].intercepted == 1){
			printk("Invalid syscall cmd, syscal=%d,cmd=%d, it has been intercepted\n",syscall,cmd);
			spin_unlock(&sys_call_table_lock  );
			spin_unlock(&my_table_lock );
			return  -EBUSY;
		}
...
static int init_function(void) {
	int i;
	//set sys_call_table as read-writable
	...
	//set exit_group
	sys_call_table[__NR_exit_group] = (long *)my_exit_group;
	
	//set my_syscall
	sys_call_table[MY_CUSTOM_SYSCALL] = my_syscall;
	
	
	//Set sys_call_table as read-oly
	set_addr_ro((unsigned long)sys_call_table); 
